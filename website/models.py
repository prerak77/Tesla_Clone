...
def create_database():
    try:
        myconn = mys.connect(host="remotemysql.com",
                             user="XbMpIqVBfR", passwd="pnykB58gL2")
        mycur = myconn.cursor()
...
def create_table_car_1():
    try:
        myconn = mys.connect(host="remotemysql.com", user="XbMpIqVBfR",
                             passwd="pnykB58gL2", database="XbMpIqVBfR")
        mycur = myconn.cursor()
        query = "create table car_details(car_model char(200) not null ,\
            performace char(200) not null,paint char(200),\
            Wheels char(200),interior char(200),Autopilot char(200),\
            selfdrive char(200),Image char(250),ID char(250))"
        mycur.execute(query)
        myconn.commit()
...
def get_data(id):
    try:
        ...
        mycur = myconn.cursor()
        query = "SELECT * FROM user_details,car_details where user_details.ID = '{}' and car_details.ID = '{}' ".format(
            id, id)
        mycur.execute(query)
        rs = mycur.fetchall()
        if len(rs) != 0:
            for i in rs:
                ...
                self_drive = i[19]
                car_img = i[20]
                Data = [ID, f_name, l_name, email, number, c_name, c_no, exp, cvv, add, city,
                        country, model, motor, colour, tyre, interior, autopiolt, self_drive, car_img]
                return Data
        else:
            return 'False'
    except Exception as e:
